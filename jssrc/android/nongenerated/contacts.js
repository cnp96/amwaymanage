...
function endSwipe(source, x, y) {
    ...
    try {
        kony.timer.schedule("resetSwipe", resetSwipe, 0.5, false);
    } catch (e) {
        kony.print("Error resetting swipe.");
    }
...
function resetSwipe() {
    swiping = false;
    try {
        kony.timer.cancel("resetSwipe");
    } catch (e) {
        kony.print("Error cancelling reset swipe.");
    }
}